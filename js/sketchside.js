...
function setup() {
  createCanvas(550, 412).parent('canvasContainer');
...
function newstyle() {
  // Create a new Style Transfer method with a defined style.
  // We give the video as the second argument
  style = ml5.styleTransfer('models/psycanvas', video, modelLoaded);
}

function draw(){
  // Switch between showing the raw camera or the style
  if (isTransfering) {
    image(resultImg, 0, 0, 550, 412);
  } else {
    image(video, 0, 0, 550, 412);
  }
}
...
function goFullScreen(){
	...
	document.getElementById("defaultCanvas0").style.width = 1600;
	document.getElementById("defaultCanvas0").style.height = 1200;
